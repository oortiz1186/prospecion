...
export async function POST(req:NextRequest){
  try{
    ...
    if(!response.ok){
      ...
      let details=raw;
      try{const parsed=JSON.parse(raw);details=parsed?.error?.message||raw;}catch{}
      return NextResponse.json({error:`Gemini respondió con error (${response.status})`,details,model},{status:502});
    }
